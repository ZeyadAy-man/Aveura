...
function Model({ currentSection = 0, sectionProgress = 0, onPartsDetected, ...props }) {
  ...
  const sharedMaterials = useMemo(() => {
    ...
    const mtMat = new THREE.MeshStandardMaterial({
      ...
      metalness: 1,
      roughness: 0.2,
      envMapIntensity: qualitySettings.envMapIntensity,
      side: sideMap[qualitySettings.side] || THREE.FrontSide
    });
    ...
    const plMat = new THREE.MeshPhysicalMaterial({
      ...
      metalness: 0,
      roughness: 0.15,
      clearcoat: qualitySettings.clearcoat,
      clearcoatRoughness: qualitySettings.clearcoatRoughness,
      sheen: 1,
      sheenRoughness: 0.3,
      ...
      envMapIntensity: qualitySettings.envMapIntensity,
      reflectivity: qualitySettings.reflectivity,
      side: sideMap[qualitySettings.side] || THREE.FrontSide
    });
    ...
    const diamondMatSimple = new THREE.MeshPhysicalMaterial({
      ...
      roughness: 0,
      transmission: 1,
      thickness: qualitySettings.thickness,
      ior: qualitySettings.ior,
      reflectivity: qualitySettings.reflectivity,
      clearcoat: qualitySettings.clearcoat,
      clearcoatRoughness: qualitySettings.clearcoatRoughness,
      transparent: true,
  ...
  }, [col, gpuData.tier, gpuData.deviceType]);
  ...
  const transmissionProps = useMemo(() => {
    ...
    return {
      ...
      clearcoat: qualitySettings.clearcoat,
      clearcoatRoughness: qualitySettings.clearcoatRoughness,
      attenuationDistance: qualitySettings.attenuationDistance,
  ...
  const n = nodes;
  const qualitySettings = getQualitySettings(gpuData.tier, gpuData.deviceType);
  const useFallback = qualitySettings.useFallback;
      ...
      {Object.entries(n).map(([name, node]) => {
        ...
        return isDiamond && !useFallback ? (
          <TransmissionMesh
            ...
            castShadow={qualitySettings.castShadow}
            receiveShadow={qualitySettings.receiveShadow}
          />
        ) : (
          <MeshComponent
            key={name}
